...
- 启动后台前端服务器和客户端前端服务器。客户端有账号和密码(都在SQL文件中) ，后台系统有超级管理员账号与密码和普通管理员账号与密码(都在SQL文件中) 。客户端端口为3000，后台系统端口为3001。因为项目中使用了qiniu云对象储存配置上传空间，如若需要，需在配置文件中配置自己的域名以及信息（已经加密脱敏）。
